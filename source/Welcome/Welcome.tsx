...
const TABS: { key: string; labelKey: string }[] = [
  { key: 'welcome', labelKey: 'welcome' },
  { key: 'changelog', labelKey: 'changelogTitle' },
  { key: 'tutorial', labelKey: 'tutorialTitle' },
  { key: 'faq', labelKey: 'faqTitle' },
];

const WELCOME_FEATURES = [
  'featureAutoSniff',
  'featureFilter',
  'featureBatchDownload',
  'featureCopyUrl',
];
...
function Welcome(): ReactElement {
  ...
  const renderTabPanel = (key: string): ReactNode => {
    switch (key) {
      case 'welcome':
        return (
          <section className={styles.section}>
            <div className={styles.sectionHeader}>
              <h2 className={styles.sectionTitle}>{t('welcomeTitle')}</h2>
            </div>
            <div className={styles.sectionBody}>
              <p className={`${styles.text} ${styles.preline}`}>
                {t('welcomeIntro')}
              </p>
              <h3 className={styles.subTitle}>{t('welcomeFeaturesTitle')}</h3>
              <ul className={styles.featureList}>
                {WELCOME_FEATURES.map((feature) => (
                  <li key={feature} className={styles.featureItem}>
                    ...
                    <span className={styles.featureText}>{t(feature)}</span>
                  </li>
                ))}
              </ul>
            </div>
  ...
  return (
    <div className={styles.welcome}>
      ...
      <div className={styles.tabPanel}>{renderTabPanel(activeTab)}</div>

      <footer className={styles.footer}>
        <span className={styles.version}>CoolHusky v{version}</span>
        <a
